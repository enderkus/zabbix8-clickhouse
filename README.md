...
- **ClickHouse is not supported as a history backend for Zabbix proxies**,
  only for the server.
- Supported value types map to dedicated tables: numeric float → `history`,
  numeric unsigned → `history_uint`, character → `history_str`, log →
  `history_log`, text → `history_text`, JSON → `history_json` (ClickHouse
  doesn't accept JSON arrays, which the schema accounts for).
...
data** to see values already flowing in — they're being served straight out
of ClickHouse.
